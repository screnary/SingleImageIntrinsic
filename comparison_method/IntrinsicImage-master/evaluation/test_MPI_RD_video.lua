...
--wzj 202103, test on MPI refined_final, for video temporal consistency
--wzj 201907, trained on MPI refined data
--imgPath = '../../datasets/MPI/MPI-main-input-300/'
imgPath = '../../datasets/MPI/refined_final/MPI-main-clean/'
-- saveDir = '../results/MPI_main_SceneSplit_front_combine/'
saveDir = '../results/test/'

local n_W = 716 --768 --512
local n_H = 304 --328 --218
--local file_name = '../../datasets/MPI/MPI_main_sceneSplit-300-test.txt'
local file_name = '../../datasets/MPI/refined_gs/MPI_main_sceneSplit-fullsize-NoDefect-test.txt'
local files = {}
...
model = torch.load('../netfileswzj/model_MPI_main_SceneSplit_front_combine_best.net')
model = model:cuda()
--model:training()
...
for _,file in ipairs(files) do
	...
	local savColor = string.gsub(file,'../../datasets/MPI/refined_final/',saveDir..'RD_')
	print('file: '..file)
        print('savColor0: ' .. savColor)
	savColor = saveDir.."/RD_MPI-main-clean-video/"..tostring(n)..".png"
