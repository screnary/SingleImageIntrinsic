...
model_guidance = torch.load(projroot..'/netfileswzj/model_MPI_main_SceneSplit_front_guidance_best_joint.net')
model_intrinsic = torch.load(projroot..'/netfileswzj/model_MPI_main_SceneSplit_front_direct_best_joint.net')
...
postfix = 'MPI_main_SceneSplit_front_combine'
max_iters = 10
batch_size = 2 --16
...
local file = projroot..'/training/train_MPI_main_SceneSplit_front_combine.lua'
...
local file = dataroot..'/MPI_main_sceneSplit-300-train.txt'
local trainSet = {}
local f = io.open(file, "rb")
while true do
  local line = f:read()
...
local file = dataroot..'/MPI_main_sceneSplit-300-test.txt'
--local file = dataroot..'/MPI_main_sceneSplit-fullsize-NoDefect-test.txt'
local testSet = {}
local f = io.open(file, "rb")
while true do
  local line = f:read()
...
step = function(batch_size)
  ...
  for t = 1,trainsetSize,batch_size do
    ...
    local feval = function(x_new)
      ...
      local loss = 0
      for i = 1,size do
        local inputFile =  dataroot.."/MPI-main-input-300/"..trainSet[t+i-1]
    ...
    if iter % 10 == 0 then
      ...
      local size = 2
      for i = 1,size do
        local inputFile = dataroot.."/MPI-main-input-300/"..testSet[testCount]
        -- print(inputFile)
