...
postfix = 'MPI_main_SceneSplit_front_direct'
max_iters = 40
batch_size = 2  --16
...
local dataroot = '/home/wzj/WZJ/Works/Intrinsic/intrinsic-project/datasets/MPI/refined_gs/'

local file = '/home/wzj/WZJ/Works/Intrinsic/intrinsic-project/IntrinsicImage-master/training/train_MPI_main_SceneSplit_front_direct.lua'
--'/raid/qingnan/codes/qingnan_codes/train_MPI_main_SceneSplit_front_direct.lua'
...
local file = '/home/wzj/WZJ/Works/Intrinsic/intrinsic-project/datasets/MPI/MPI_main_sceneSplit-300-train.txt'
--'/raid/qingnan/data/MPI_main_sceneSplit-300-train.txt'
local trainSet = {}
local f = io.open(file, "rb")
while true do
  local line = f:read()
...
local file = '/home/wzj/WZJ/Works/Intrinsic/intrinsic-project/datasets/MPI/MPI_main_sceneSplit-fullsize-NoDefect-test.txt'
--'/raid/qingnan/data/MPI_main_sceneSplit-fullsize-NoDefect-test.txt'
local testSet = {}
local f = io.open(file, "rb")
while true do
  local line = f:read()
...
step = function(batch_size)
  ...
  for t = 1,trainsetSize,batch_size do
    ...
    local feval = function(x_new)
      ...
      local loss= 0
      for i = 1,size do
        local inputFile =  dataroot.."MPI-main-input-300/"..trainSet[order[t+i-1]]

        local albedoFile = string.gsub(inputFile,'input','albedo')
        local shadingFile = string.gsub(inputFile,'input','shading')
        --print("input, albedo, shading filename:")
        --print(inputFile)
        --print(albedoFile)
        --print(shadingFile)

        local tempInput = image.load(inputFile)
        local tempAlbedo = image.load(albedoFile)
        local tempShading = image.load(shadingFile)
    ...
    if iter % 10 == 0 then
      local testloss= 0
      for i = 1,size do
        local inputFile = dataroot.."MPI-main-clean/"..testSet[testCount]

        local albedoFile = string.gsub(inputFile,'clean','albedo')
        local shadingFile = string.gsub(inputFile,'clean','shading')

	local n_W = 512 --768 --512
	local n_H = 218 --328 --218
        local tempInput = image.load(inputFile)
        tempInput = image.scale(tempInput, n_W, n_H, bilinear)
        local tempAlbedo = image.load(albedoFile)
        tempAlbedo = image.scale(tempAlbedo, n_W, n_H, bilinear)
        local tempShading = image.load(shadingFile)
        tempShading = image.scale(tempShading, n_W, n_H, bilinear)
...
netfiles = '/home/wzj/WZJ/Works/Intrinsic/intrinsic-project/IntrinsicImage-master/netfileswzj/'
--'/raid/qingnan/codes/intrinsic/netfiles/'
timer = torch.Timer()
