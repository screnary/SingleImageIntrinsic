function [enhanced_result, R_color, R, L, I_rec] = srie(input_image)
% A wrapper of SRIE (Simultaneous Reflectance and Illumination Estimation)
% 
% -------------------------------------------------------------------------
...
if size(img,3)>1
    R_color = hsv2rgb(cat(3,HSV(:,:,1), HSV(:,:,2),R));
else
    R_color = R;
end

I_rec = hsv2rgb(cat(3,HSV(:,:,1), HSV(:,:,2), R.*L/255));
