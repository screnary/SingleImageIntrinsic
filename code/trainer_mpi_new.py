...
from networks_mpi_new import get_generator, get_discriminator, GANLoss, DivergenceLoss, PerspectiveLoss
from networks_mpi_new import Grad_Img_v1 as Grad_Img
import pytorch_ssim
import copy
import pdb
...
class Trainer_Basic(nn.Module):
    ...
    def forward(self):
        ...
        fake_s, fake_r, fd_loss, fake_i = self.gen_split(self.real_i)[:4]
        self.fake_s = fake_s
        self.fake_r = fake_r
    ...
    def loss_basic_computation(self):
        ...
        if weight.gradient_w > 0:
            self.loss_grad_s = 0.15 * (self.criterion_mse(fake_s_gradx,
                                                         self.real_s_gradx)
                                     +self.criterion_mse(fake_s_grady,
                                                         self.real_s_grady)) * weight.lambda_b_w
            self.loss_grad_r = 0.15 * (self.criterion_mse(fake_r_gradx,
                                                         self.real_r_gradx)
                                     +self.criterion_mse(fake_r_grady,
                                                         self.real_r_grady)) * weight.lambda_r_w
            self.loss_grad_s += 0.85 * (self.criterion_idt(fake_s_gradx,
                                                    self.real_s_gradx)
                                +self.criterion_idt(fake_s_grady,
                                                    self.real_s_grady)) * weight.lambda_b_w
            self.loss_grad_r += 0.85 * (self.criterion_idt(fake_r_gradx,
                                                    self.real_r_gradx)
