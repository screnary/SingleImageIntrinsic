...
#from networks_mpi_RD_v9 import get_generator, get_discriminator, GANLoss, DivergenceLoss, PerspectiveLoss
#from networks_mpi_RD_v9 import Grad_Img_v1 as Grad_Img
...
class Trainer_Basic(nn.Module):
    ...
    def forward(self):
        ...
        fake_s, fake_r, fd_loss = self.gen_split(self.padimg(self.real_i))[:3]
        self.fake_s = fake_s[:,:,self.padedge:-self.padedge,self.padedge:-self.padedge].repeat(1, 3, 1, 1)
        self.fake_r = fake_r[:,:,self.padedge:-self.padedge,self.padedge:-self.padedge]
        self.fd_loss = fd_loss
    ...
    def set_input(self, input_data):
        ...
        valid_thre_s = s_grad.abs().mean() * 0.45
        valid_thre_r = r_grad.abs().mean() * 0.45
    ...
    def loss_basic_computation(self):
        ...
        if weight.identity_w > 0:
            self.loss_idt_i = self.criterion_idt(self.fake_i, self.real_r * self.real_s) * weight.lambda_i_w
        ...
        if weight.ssim_w > 0:
            self.loss_ssim_i = (1 - self.criterion_ssim(self.fake_i, self.real_r * self.real_s)) * weight.lambda_i_w
        ...
        if weight.gradient_w > 0:
            ...
            self.loss_grad_s += 0. * (self.criterion_idt(self.fake_s_gradx,
                                                    self.real_s_gradx)
                                +self.criterion_idt(self.fake_s_grady,
                                                    self.real_s_grady)) * weight.lambda_b_w
            self.loss_grad_r += 0. * (self.criterion_idt(self.fake_r_gradx,
                                                    self.real_r_gradx)
        ...
        if weight.divergence_w > 0:
            #self.loss_feature_divergence = self._compute_divergence_loss()
    ...
    def _compute_divergence_loss(self):
        fea_base = self.gen_split.encoder_shared(self.real_i)
        fea_s = self.gen_split.encoder_a(fea_base['mid2'])
        fea_r = self.gen_split.encoder_b(fea_base['mid2'].clone())
        fea_s.update(fea_base)
        fea_r.update(fea_base)
        feature_div = self.criterion_fd(fea_r, fea_s,
                                         detail_weights=self.opt.optim.div_detail_dict)
        return feature_div

    def _compute_perspective_loss(self):
        fea_fake_r_base = self.gen_split.encoder_shared(self.fake_r)
        fea_real_r_base = self.gen_split.encoder_shared(self.real_r)
        fea_fake_s_base = self.gen_split.encoder_shared(self.fake_s)
        fea_real_s_base = self.gen_split.encoder_shared(self.real_s)

        fea_fake_r = self.gen_split.encoder_b(fea_fake_r_base['mid2'])
        fea_real_r = self.gen_split.encoder_b(fea_real_r_base['mid2'])
        fea_fake_s = self.gen_split.encoder_a(fea_fake_s_base['mid2'])
        fea_real_s = self.gen_split.encoder_a(fea_real_s_base['mid2'])

        fea_fake_r.update(fea_fake_r_base)
        fea_fake_s.update(fea_fake_s_base)
        fea_real_r.update(fea_real_r_base)
        fea_real_s.update(fea_real_s_base)
    ...
    def _compute_fea_extract_loss(self):
        fea_extract_s = self.gen_split.encoder_shared(self.input_i)
        fea_real_s = self.gen_split.encoder_shared(self.real_s)
        fea_extract_r = self.gen_split.encoder_shared(self.input_i)
        fea_real_r = self.gen_split.encoder_shared(self.real_r)
    ...
    def update_optim_weights(self, ep):
        ...
        if ep < 0 and self.opt.optim.ssim_w > 0 and not self.opt.continue_train:
