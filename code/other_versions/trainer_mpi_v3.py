...
from networks import get_generator, get_discriminator, GANLoss, DivergenceLoss, PerspectiveLoss, DistanceLoss, Grad_Img
import pytorch_ssim
import copy
import pdb
...
"""MPI data, use L1 loss rather than MSE loss"""
...
class Trainer_Basic(nn.Module):
    ...
    def loss_basic_computation(self):
        ...
        if weight.divergence_w > 0:
            div_detail = weight.div_detail_dict  # 'low', 'mid', 'deep', 'out'
    ...
    # [wzj] update weights during training
    def update_optim_weights(self, ep):
        weight_old = self.weights  # weight for optim settings
        if ep < 20:
            self.opt.optim.identity_w = weight_old.ssim_w
            self.opt.optim.ssim_w = weight_old.identity_w
        else:
            self.opt.optim.identity_w = weight_old.identity_w
            self.opt.optim.ssim_w = weight_old.ssim_w
        print('optims: identyty_w={},ssim_w={}'.format(
