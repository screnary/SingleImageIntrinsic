...
except ImportError:  # will be 3.x series  tesrtt13123 v21412
    pass
...
class CrossGenMS(CrossGen):
    def __init__(self, gen_opt):
        ...
        self.decoder_a = DecoderMS_bak(self.input_dim, dim=self.dim,
                                   output_dim=self.output_dim1,  # s
                                   n_layers=self.n_layers, pad_type=self.pad_type, activ=self.activ,
                                   norm=self.norm, decoder_mode=self.decoder_mode)
        self.decoder_b = DecoderMS_bak(self.input_dim, dim=self.dim,
...
class DecoderMS(nn.Module):
    ...
    def forward(self, input_x, feat_dict):
        #x = feat_dict['out'] # 512
        #x = self.fuse_out(x)
        #x = self._fuse_feature(x, feat_dict['deep']) # 256
        #x = self.fuse_deep(x)
        #x = self._fuse_feature(x, feat_dict['mid']) # 128
        #x = self.fuse_mid(x)
        #x = self._fuse_feature(x, feat_dict['low']) # 64
        #x = self.fuse_low(x)
        x = feat_dict['low']
...
class DecoderMS_bak(nn.Module):
    def __init__(self, input_dim, dim, output_dim, n_layers, pad_type, activ, norm, decoder_mode='Basic'):
        """output_shape = [H, W, C]"""
        super(DecoderMS_bak, self).__init__()
