...
class Trainer_Basic(nn.Module):
    ...
    def set_input(self, input_data):
        ...
        valid_thre_r = r_grad.abs().mean() * 0.48
    ...
    def loss_basic_computation(self):
        ...
        if weight.gradient_w > 0:
            self.loss_grad_s = 0.5* self.criterion_mse(fake_s_grad,
                                                  self.real_s_grad) * weight.lambda_b_w
            self.loss_grad_r = 0.5 * self.criterion_mse(fake_r_grad,
                                                  self.real_r_grad) * weight.lambda_r_w
            self.loss_grad_s += self.criterion_idt(fake_s_grad,
                                                   self.real_s_grad) * weight.lambda_b_w
            self.loss_grad_r += self.criterion_idt(fake_r_grad,
                                                   self.real_r_grad) * weight.lambda_r_w
