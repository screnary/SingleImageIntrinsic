...
from networks import get_generator, get_discriminator, GANLoss, DivergenceLoss, PerspectiveLoss, DistanceLoss
from networks import Grad_Img_v1 as Grad_Img
import pytorch_ssim
import copy
import pdb
...
class Trainer_Basic(nn.Module):
    ...
    def loss_basic_computation(self):
        ...
        if weight.preserve_info_w > 0:
            ...
            self.loss_preserve_info = (diff_avg_r * weight.lambda_r_w +\
                                       diff_avg_s * weight.lambda_b_w) *\
        ...
        if weight.gradient_w > 0:
            self.loss_grad_s = 0.35 * (self.criterion_mse(fake_s_gradx,
                                                         self.real_s_gradx)
                                     +self.criterion_mse(fake_s_grady,
                                                         self.real_s_grady)) * weight.lambda_b_w
            self.loss_grad_r = 0.35 * (self.criterion_mse(fake_r_gradx,
                                                         self.real_r_gradx)
                                     +self.criterion_mse(fake_r_grady,
                                                         self.real_r_grady)) * weight.lambda_r_w
            self.loss_grad_s += 0.65 * (self.criterion_idt(fake_s_gradx,
                                                    self.real_s_gradx)
                                +self.criterion_idt(fake_s_grady,
                                                    self.real_s_grady)) * weight.lambda_b_w
            self.loss_grad_r += 0.65 * (self.criterion_idt(fake_r_gradx,
                                                    self.real_r_gradx)
                                +self.criterion_idt(fake_r_grady,
                                                    self.real_r_grady)) * weight.lambda_r_w
    ...
    def _compute_divergence_loss(self):
        ...
        feature_diff = self.criterion_perspective(fea_r, fea_s,
                                                  detail_weights=self.opt.optim.div_detail_dict)
        return 1.0 - feature_diff
    ...
    def update_learning_rate(self):
        ...
        if lr <= 1.1e-6 and self.opt.use_wave_lr:  # reset lr
    ...
    def update_optim_weights(self, ep):
        ...
        if ep < 3 and self.opt.optim.ssim_w > 0:
