...
import my_data
from configs.intrinsic_mpi_v5 import opt
import trainer_mpi_v3 as Trainer
from utils import save_eval_images, check_dir, visualize_inspector
import numpy as np
import pdb

"""
v3: use vgg19; use L1 loss rather than MSE loss
v5: use L1 loss, no gradient, with SSIM loss (change during training)
"""
...
def basic_settings():
    ...
    if 'MPI' in opt.data.name:
        train_dataset = my_data.DatasetIdMPI(opt.data, is_train=True, cropped=False)
        test_dataset = my_data.DatasetIdMPI(opt.data, is_train=False, cropped=False)
...
def train():
    ...
    for ep in range(opt.train.total_ep):  # total_ep=300
        ...
        # update ssim and identity loss weight [wzj]
        s['model_trainer'].update_optim_weights(ep)  # idt_w and ssim_w
        ...
        if epoch % opt.train.save_per_n_ep == 0:
            ...
            if opt.train.trainer_mode == 'Basic':
                with open(opt.logger.log_dir + '/all_eval_log-' + str(start_epoch - 1 + opt.train.total_ep) + '.txt', 'w') as f:
                    f.write(
                        'epoch: total_loss: idt_loss: ssim_loss: grad_loss: fea_divergence: perspective: fea_extract\n')
