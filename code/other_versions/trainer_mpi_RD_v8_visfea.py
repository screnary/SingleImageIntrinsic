...
from networks_mpi_RD_v8 import get_generator, get_discriminator, GANLoss, DivergenceLoss, PerspectiveLoss
from networks_mpi_RD_v8 import Grad_Img_v1 as Grad_Img
import pytorch_ssim
import copy
import pdb
...
RD: refined data
RD_v8_visfea: for feature visualization
"""
...
class Trainer_Basic(nn.Module):
    ...
    def forward_feas(self):
        self.gen_split.eval()
        encoder_s = self.gen_split.encoder_a
        encoder_r = self.gen_split.encoder_b
        feats_s = encoder_s(self.real_i)
        feats_r = encoder_r(self.real_i)
        feats_out_s = feats_s['out']
        feats_out_r = feats_r['out']
        fea_s = torch.unsqueeze(torch.mean(feats_out_s.view([feats_out_s.size(1), -1]), dim=1), dim=0)
        fea_r = torch.unsqueeze(torch.mean(feats_out_r.view([feats_out_r.size(1), -1]), dim=1), dim=0)

        self.gen_split.train()
        # pdb.set_trace()
        return fea_s.detach().cpu().numpy(), fea_r.detach().cpu().numpy()
        # save into npy files
    ...
    def loss_basic_computation(self):
        ...
        if weight.gradient_w > 0:
            self.loss_grad_s = 0.85 * (self.criterion_mse(self.fake_s_gradx,
                                                         self.real_s_gradx)
                                     +self.criterion_mse(self.fake_s_grady,
                                                         self.real_s_grady)) * weight.lambda_b_w
            self.loss_grad_r = 0.85 * (self.criterion_mse(self.fake_r_gradx,
                                                         self.real_r_gradx)
                                     +self.criterion_mse(self.fake_r_grady,
                                                         self.real_r_grady)) * weight.lambda_r_w
            self.loss_grad_s += 0.15 * (self.criterion_idt(self.fake_s_gradx,
                                                    self.real_s_gradx)
                                +self.criterion_idt(self.fake_s_grady,
                                                    self.real_s_grady)) * weight.lambda_b_w
            self.loss_grad_r += 0.15 * (self.criterion_idt(self.fake_r_gradx,
                                                    self.real_r_gradx)
    ...
    def _compute_divergence_loss(self):
        fea_r = self.gen_split.encoder_b(self.real_i)
        fea_s = self.gen_split.encoder_a(self.real_i)
    ...
    def update_optim_weights(self, ep):
        ...
        if ep < 2 and self.opt.optim.ssim_w > 0 and not self.opt.continue_train:
