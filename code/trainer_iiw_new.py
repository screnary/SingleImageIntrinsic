...
from networks_iiw import get_generator, JointLoss, PerspectiveLoss
from networks_iiw import Grad_Img_v1 as Grad_Img
import pytorch_ssim
import pdb

"""
iiw_oneway_pixsup_v7
"""
...
class Trainer_Basic(nn.Module):
    ...
    def forward(self):
        if 'oneway' in self.opt.train.mode:
            ...
            fake_s, fake_r, _ = self.gen_decompose(self.real_i)[:3]
            self.fake_s = fake_s
            self.fake_r = fake_r
            ...
            fake_s, fake_r, fd_loss = self.gen_decompose(self.real_i)[:3]
            self.fake_s = fake_s
            self.fake_r = fake_r
    ...
    def get_out_reflectance(self):
        self.mu_r = 0.4724   # for visualization
        self.std_r = 0.0738
        r_value = self.fake_r.clone()
        guide_im = self.input_i.clone()
        r_value[r_value < 1e-4] = 1e-4
        guide_im[guide_im < 1e-4] = 1e-4
        input_mean = torch.mean(self.input_i, dim=1)
        input_mean[input_mean < 1e-4] = 1e-4

        for b in range(self.fake_r.size(0)):
            mu = torch.mean(self.fake_r[b])
            std = torch.std(self.fake_r[b])
            r_value[b,0,:,:] = (self.fake_r[b,0,:,:] - mu) * self.std_r / std + self.mu_r

        r_div = r_value / input_mean
        out_reflectance = torch.zeros_like(guide_im)
        out_reflectance[:,0,:,:] = guide_im[:,0,:,:] * r_div
        out_reflectance[:,1,:,:] = guide_im[:,1,:,:] * r_div
        out_reflectance[:,2,:,:] = guide_im[:,2,:,:] * r_div

        out_shading = input_mean / r_value
        return out_reflectance, out_shading
    ...
    def get_current_visuals(self):
        ...
        out_reflectance, out_shading = self.get_out_reflectance()
        p_R = out_reflectance.clone()
        p_S = out_shading.clone()
        # rescale r values
        for b in range(out_reflectance.size(0)):
            for m in range(0,3):
                numerator = torch.dot(out_reflectance[b,m,:,:].contiguous().view(-1),
                                      self.input_i[b,m,:,:].contiguous().view(-1))
                denominator = torch.dot(out_reflectance[b,m,:,:].contiguous().view(-1),
                                        out_reflectance[b,m,:,:].contiguous().view(-1))
                alpha = numerator / denominator
                p_R[b,m,:,:] = out_reflectance[b,m,:,:] * alpha

        # rescale s values
        #pdb.set_trace()
        for b in range(out_shading.size(0)):
            for m in range(0,1):
                numerator = torch.dot(out_shading[b,m,:,:].contiguous().view(-1),
                                      self.input_i[b,m,:,:].contiguous().view(-1))
                denominator = torch.dot(out_shading[b,m,:,:].contiguous().view(-1),
                                        out_shading[b,m,:,:].contiguous().view(-1))
                alpha = numerator / denominator
                p_S[b,m,:,:] = out_shading[b,m,:,:] * alpha

        img_fake_r = utils.tensor2img(p_R)
        img_rec_s = utils.tensor2img(self.rec_s.detach().clone(), mean, std, use_norm=False)
        img_fake_s = utils.tensor2img(p_S.repeat(1,3,1,1))
