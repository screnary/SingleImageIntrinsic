...
from configs.intrinsic_mpi_RD_v8_3 import opt
#from configs.intrinsic_mpi_RD_v8_3_new import opt
import trainer_mpi_RD_v8 as Trainer
...
[2021.03.06] for ablation studies: remove gradient loss, remove SSIM loss, remove gradient and SSIM loss
"""
...
# pdb.set_trace()
opt.optim.ssim_w = FLAGS.w_ssim
opt.optim.gradient_w = FLAGS.w_grad
opt.output_root = '../ckpoints-'+opt.train.mode+'-'+opt.data.name+'-'+opt.data.split \
                  + '-w_ssim-' + str(opt.optim.ssim_w) + '-w_grad-' + str(opt.optim.gradient_w)
opt.logger.log_dir = opt.output_root + '/log/'
#<<<<<<< 2021.03.06 <<<<<<< 
...
if __name__ == '__main__':
    ...
    if FLAGS.phase=="test":
        if FLAGS.best_ep is not None:
            ...
            test(epoch=150)
    #test(epoch=200)
    # test(epoch=80)  # use this for RD dataset results
    #test(epoch=150)
