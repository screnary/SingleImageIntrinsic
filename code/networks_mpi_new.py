...
"""
new: add reconstruct net: f(R,S)=I
"""
##################################################################################
# Interfaces
##################################################################################
...
class CrossGenMS(CrossGen):
    def __init__(self, gen_opt):
        ...
        self.combiner = Combiner(input_dim=6, dim=32, output_dim=3, n_layers=2,
                                 pad_type=self.pad_type, activ=self.activ,
                                 norm=self.norm,
    ...
    def forward(self, x):
        ...
        pred_layers = torch.cat([out_a, out_b], dim=1)  # [2,6,288,288]
        out_rec = self.combiner(pred_layers)  # [2,3,288,288]
        # pdb.set_trace()
        return out_a, out_b, fd_loss, out_rec  # , [d_s1, d_s2, d_s3, d_s4]
...
class Combiner(nn.Module):
    ...
    def __init__(self, input_dim, dim, output_dim, n_layers, pad_type, activ,
                 norm, decoder_mode='Residual'):
        """output_shape = [H, W, C]"""
        super(Combiner, self).__init__()
        self.model = []

        rates = [1,1,1,1,1]  # no dilation

        if decoder_mode == 'Basic':
            for i in range(n_layers):
                _dim = input_dim if i==0 else dim
                self.model += [Conv2dBlock(_dim, dim, kernel_size=3, dilation=rates[i], padding=rates[i],
                                          pad_type=pad_type, activation=activ, norm=norm)]
        elif decoder_mode == 'Residual':
            for i in range(n_layers):
                _dim = input_dim if i==0 else dim
                self.model += [ResDilateBlock_v2(input_dim=_dim, dim=dim, output_dim=dim, rate=rates[i],
                                                 padding_type=pad_type, norm=norm)]

        # use reflection padding in the last conv layer
        # print('Decoder Conv Dim is:', dim)  # 64
        # pdb.set_trace()
        self.model += [Conv2dBlock(dim, dim, kernel_size=3, padding=1,
                                   norm=norm, activation=activ,
                                   pad_type='reflect')]
        self.model += [Conv2dBlock(dim, dim, kernel_size=1, padding=0,
                                   norm=norm, activation=activ,
                                   pad_type=pad_type)]
...
class DecoderMS(nn.Module):
    def __init__(self, input_dim, dim, output_dim, n_layers, pad_type, activ, norm, decoder_mode='Basic'):
        ...
        if decoder_mode == 'Basic':
            ...
            for i in range(n_layers):
                self.contextual_blocks += [ResDilateBlock_v2(input_dim=dim, dim=dim, output_dim=dim, rate=rates[i],
                                           padding_type=pad_type, norm=norm)]
        else:
            raise NotImplementedError
...
class ResDilateBlock_v2(nn.Module):
    # for combiner, kernelsize=3
    def __init__(self, input_dim, dim, output_dim, rate,
                 padding_type, norm, use_bias=False):
        super(ResDilateBlock_v2, self).__init__()
...
class ResDilateBlock(nn.Module):
    ...
    def build_conv_block(self, input_dim, dim, output_dim, rate,
        ...
        # dilated conv, padding = dilation_rate, when k=3, s=1, p=d
        # k=5, s=1, p=2d
        conv_block += [self.pad_layer(padding_type, padding=2*rate),
                       nn.Conv2d(dim, dim, kernel_size=5, stride=1,
                                 bias=False, dilation=rate),
                       self.norm_layer(norm, dim),
                       nn.ReLU(True)]
        conv_block += [self.pad_layer(padding_type, padding=0),
