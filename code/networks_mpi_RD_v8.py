...
class DivergenceLoss(nn.Module):
    ...
    def _compute_dist_loss(self, fea_x, fea_y):
        ...
        d_cos = torch.pow(cos, 2) * 0.5  # symmetry function; the smaller, the better
    ...
    def forward(self, features_x, features_y, detail_weights=None):
        ...
        for key in features_x.keys():
            if key=='out':
                fea_x = torch.max(torch.max(features_x[key], dim=2, keepdim=True)[0],
                                  dim=3, keepdim=True)[0]
                fea_y = torch.max(torch.max(features_y[key], dim=2, keepdim=True)[0],
                                  dim=3, keepdim=True)[0]
