...
class CrossGenMS(CrossGen):
    def __init__(self, gen_opt):
        ...
        # Feature extractor as Encoder
        self.encoder_shared = Vgg19EncoderBase(input_dim=self.input_dim,
                                               pretrained=self.pretrained)
        ...
        self.decoder_a = DecoderMS_Res(self.input_dim, dim=self.dim,
                                   output_dim=self.output_dim1,  # s
                                   n_layers=self.n_layers, pad_type=self.pad_type, activ=self.activ,
                                   norm=self.norm, decoder_mode=self.decoder_mode)
        self.decoder_b = DecoderMS_Res(self.input_dim, dim=self.dim,
    ...
    def forward(self, x):
        feats_base = self.encoder_shared(x)
        feats_a = self.encoder_a(feats_base['mid2'])
        feats_b = self.encoder_b(feats_base['mid2'].clone())
        #pdb.set_trace()
        ...
        if self.mode == 'direct':
            features_a = feats_a
            features_b = feats_b
            features_a.update(feats_base)
            features_b.update(feats_base)
        else:
            raise NotImplementedError
...
class CrossGenMSIIW(CrossGen):
    ...
    def forward(self, x):
        ...
        out_a = self.decode(x, 'a', features_a)
        out_b = self.decode(x, 'b', features_b)
        out_a = self.sigmoid_func(out_a)
        out_b = self.sigmoid_func(out_b)
        # d_s1, d_s2, d_s3, d_s4 = self.compute_fea_diverse(feats_a, feats_b)
        return out_a, out_b, fd_loss  # , [d_s1, d_s2, d_s3, d_s4]
...
class Vgg19EncoderBase(nn.Module):
    def __init__(self, input_dim, pretrained):
        super(Vgg19EncoderBase, self).__init__()
        features = list(vgg19(pretrained=pretrained, in_channels=input_dim).features)
        self.features = nn.ModuleList(features)[0:17]
...
class Vgg19EncoderMS(nn.Module):
    def __init__(self, input_dim, pretrained):
        ...
        features = list(vgg19(pretrained=pretrained, in_channels=input_dim).features)
        self.features = nn.ModuleList(features)[17:]

    def forward(self, x):
        result_dict = {}
        layer_names = ['conv4_1', 'conv4_2', 'conv4_3', 'conv4_4',
                       'conv5_1', 'conv5_2', 'conv5_3', 'conv5_4']
        idx = 0
        for ii, model in enumerate(self.features):
            x = model(x)
            if ii in {2, 4, 6 , 8, 11, 13, 15, 17}:  # {19, 21, 23, 25, 28, 30, 32, 34}:
                result_dict[layer_names[idx]] = x
                idx += 1
...
class DecoderMS(nn.Module):
    def __init__(self, input_dim, dim, output_dim, n_layers, pad_type, activ, norm, decoder_mode='Basic'):
        ...
        #                            pad_type=pad_type, activation=activ, norm=norm)
        self.fuse_deep = Conv2dBlock(512, 224, kernel_size=3, stride=1, padding=1,
                                     pad_type=pad_type, activation=activ, norm=norm)
        self.fuse_mid2 = Conv2dBlock(480, 160, kernel_size=3, stride=1, padding=1,
                                     pad_type=pad_type, activation=activ, norm=norm)
        self.fuse_mid = Conv2dBlock(288, 128, kernel_size=3, stride=1, padding=1,
                                    pad_type=pad_type, activation=activ, norm=norm)
        self.fuse_low = Conv2dBlock(192, 64, kernel_size=3, stride=1, padding=1,
                                    pad_type=pad_type, activation=activ, norm=norm)
        self.fuse_input = Conv2dBlock(64 + input_dim, dim, kernel_size=3, stride=1, padding=1,
                                      pad_type=pad_type, activation=activ, norm=norm)
...
class ResDilateBlock(nn.Module):
    ...
    def build_conv_block(self, input_dim, dim, output_dim, rate,
        ...
        # k=5, s=1, p=2d
        conv_block += [self.pad_layer(padding_type, padding=1*rate),
...
class DivergenceLoss(nn.Module):
    ...
    def forward(self, features_x, features_y, detail_weights=None):
        ...
        for key in features_x.keys():
            if key=='out':
                batch_size = features_x[key].size(0)
...
class JointLoss(nn.Module):
    ...
    def BatchRankingLoss(self, prediction_R, judgements_eq, judgements_ineq, random_filp):
        ...
        if judgements_eq.size(1) > 2:
            ...
            R_vec = prediction_R.contiguous().view(num_channel, -1)
