...
for syn_i = 1:length(synnames)
    ...
    DumpDir = ['../datasets/', 'MPI', '/temp_refine_2/'];
    OutDir = ['../datasets/', 'MPI', '/refined_v3/'];
    ...
    albedo_valid_masks = load(['./albedo_masks/', synname, '.mat']);
    albedo_valid_masks = albedo_valid_masks.new_valids;
    shading_invalid_masks = load(['./shading_masks/', synname, '_dilated.mat']);
    shading_invalid_masks = shading_invalid_masks.new_masks;
    ...
    %     L_arec_valid = L_a_rec(s_valid_mask & a_valid_mask);
        L_arec_valid = L_a_rec(albedo_valid_masks{iter+1});
    ...
    %         L_s_new = L_s_;
            % % fill hole using LLE
            invalid_mask = shading_invalid_masks{iter+1};
            tic
            L_s_refine = LLE_smoothing(L_s_, input_filtered, invalid_mask, 20); % use this, K=15
