...
opt.which_epoch = 6                  # ['latest'/ 10] Which epoch to recover
...
opt.train.mode = 'iiw-oneway+pixsupv2'          # [iiw-oneway / iiw-cross+fd+perc]  train generator mode [self-sup/none], [self-sup/cross-deconv/self-sup-ms]
...
opt.optim.n_iter_decay = 1                # iterations to linearly decay learning rate to zero [100]
...
opt.optim.lr_g = 0.00005  # 0.00002                     # initial learning rate for generator, [0.0001]
...
opt.optim.shading_max = 0.93  # 0.97
...
opt.optim.w_ss_dense = 1.5                  # dense shading smooth loss         [4.0]
...
if 'oneway' in opt.train.mode:
    ...
    opt.optim.pixel_suppress_w = 0.5 # 0.1
