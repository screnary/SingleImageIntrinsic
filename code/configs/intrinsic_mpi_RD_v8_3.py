...
opt.gpu_ids = [3]                           # gpu ids, [0/2/3] or their combination
...
opt.train.mode = 'v8-3_self-sup+ms'                 # train generator mode [self-sup/none], [self-sup/cross-deconv/self-sup-ms]
...
opt.train.save_epoch_freq = 10               # the starting epoch count: epoch_count += save_freq
...
opt.optim.n_iter_decay = 40  # 30                # iterations to linearly decay learning rate to zero [100]
...
opt.optim.div_detail_dict = {'low':0.01, 'mid':0.1, 'mid2':0.5, 'deep': 0.7, 'out': 1.0}
...
opt.optim.divergence_w = 0.0 #1.5                # 'fd', weight of feature divergence loss, [1.0]
opt.optim.perspective_w = 0.0 #1.0               # 'pers', feature perspective loss
