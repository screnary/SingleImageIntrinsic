...
opt.train.mode = 'self-sup+ms+fd+pers+vgg19_v7'                 # train generator mode [self-sup/none], [self-sup/cross-deconv/self-sup-ms]
...
opt.optim.n_iter_decay = 25  # 30                # iterations to linearly decay learning rate to zero [100]
...
opt.optim.ssim_w = 1.0                      # weight of image SSIM loss
opt.optim.gradient_w = 2.5                  # weight of image gradient loss, [1.0]

opt.optim.divergence_detail = [0.2, 0.4, 1.0, 2.0]  # [low, mid, deep, out]
opt.optim.div_detail_dict = {'low':0.05, 'mid':0.05, 'deep': 0.2, 'out':0.7}
opt.optim.div_detail_dict_equal = {'low':1.0, 'mid':1.0, 'deep': 1.0, 'out':1.0}

opt.optim.divergence_w = 1.0                # 'fd', weight of feature divergence loss, [1.0]
...
opt.optim.lambda_b_w = 1.0                  # weight for cycle loss (B, R -> I -> B, R) B
