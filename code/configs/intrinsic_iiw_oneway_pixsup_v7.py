...
opt.which_epoch = 100                  # ['latest'/ 10] Which epoch to recover
...
opt.data.use_norm = True                           # whether use normalization [True/False], data loaders do not use norm
...
opt.train.mode = 'iiw_v7-oneway+pixsupv2+vgg19'          # [iiw-oneway / iiw-cross+fd+perc]  train generator mode [self-sup/none], [self-sup/cross-deconv/self-sup-ms]
...
opt.train.save_per_n_ep_train = 5 #1  # 10
opt.train.epoch_count = 1 if not opt.continue_train else opt.which_epoch+1    # the starting epoch count: epoch_count += save_freq
opt.train.total_ep = 10  #15(final, use 9&12) 250, MIT-50       # how many epochs to train from now
...
opt.model.gen.fd_cosw = 0.9 #0.99                # for DivergenceLoss computation
opt.model.gen.fd_normw = 0.1 #0.01
...
opt.optim.n_iter_decay = 5  # 1                # iterations to linearly decay learning rate to zero [100]
...
opt.optim.lr_g = 0.00005                     # initial learning rate for generator, [0.0001]
...
opt.optim.gradient_w = 0.1                  # weight of image gradient loss, [1.0]

opt.optim.divergence_detail = [0.2, 0.4, 1.0, 2.0]  # [low, mid, deep, out]

opt.optim.div_detail_dict = {'low':0., 'mid':0., 'mid2':0., 'deep': 0.2, 'out': 0.4}
opt.optim.div_detail_dict_equal = {'low':0.2, 'mid':0.2, 'mid2':0.5, 'deep': 1.0, 'out':1.0}
...
opt.optim.divergence_w = 0.1                  # weight of feature divergence loss, [1.0]
opt.optim.perspective_w = 0.5               # feature perceptual loss

opt.optim.threshold = 0.0035
opt.optim.thre_decay = 0.45  # 0.65
opt.optim.albedo_min = 0.05
opt.optim.shading_max = 0.95

opt.optim.iiw_cross_w = 0.1  # 0.01               # cross loss total weight, set to be smaller!
opt.optim.preserve_info_w = 3.0             # regularization
opt.optim.pixel_suppress_w = 10.0  #1e-6              # penalize invalid pixels
...
if 'oneway' in opt.train.mode:
    ...
    opt.optim.pixel_suppress_w = 10.0
