...
opt.which_epoch = 10                  # ['latest'/ 10] Which epoch to recover
...
opt.train.mode = 'iiw-oneway+pixsupv2-v4'          # [iiw-oneway / iiw-cross+fd+perc]  train generator mode [self-sup/none], [self-sup/cross-deconv/self-sup-ms]
opt.train.trainer_mode = ''            # [Basic/GAN]
opt.train.pool_size = 50                    # image pool size
...
opt.optim.n_iter_decay = 9                  # iterations to linearly decay learning rate to zero [100]
...
opt.optim.perspective_w = 0.5               # feature perspective loss

opt.optim.threshold = 0.05
opt.optim.thre_decay = 0.65  # 0.65
opt.optim.albedo_min = 0.05  # 0.03
opt.optim.shading_max = 0.94  # 0.97

opt.optim.iiw_cross_w = 0.01                 # cross loss total weight, set to be smaller!
opt.optim.preserve_info_w = 5.0             # regularization
opt.optim.pixel_suppress_w = 0.001              # penalize invalid pixels
...
opt.optim.w_rs_local = 1.5                  # multiscale smooth for reflectance [1.0]
opt.optim.w_reconstr_real = 0.0             # reconstruction Image loss weight  [2.0]
# opt.optim.w_rs_dense = 2.0
opt.optim.w_ss_dense = 2.0 # 2.0                 # dense shading smooth loss         [4.0]
opt.optim.w_IIW = 2.75                      # [4.0]
...
if 'oneway' in opt.train.mode:
    ...
    opt.optim.pixel_suppress_w = 1.5  # 0.01
