...
opt.which_epoch = 250                  # Which epoch to recover
...
opt.data.name = 'MPI-main'                               # [MPI-main] the name of dataset [MPI, MIT] MPI-main
...
opt.data.batch_size = 2                             # batch size [8]
...
opt.data.preprocess = 'resize_and_crop_refine'             # pre-process[resize_and_crop/crop/scale_width/scale_width_and_crop] [resize_and_crop]
...
opt.data.image_mean = (0.5, 0.5, 0.5)      # image mean value  (0.4914, 0.4822, 0.4465), (0.5, 0.5, 0.5)
#opt.data.image_mean = (0.4078, 0.4588, 0.4824)      # image mean value  (0.4914, 0.4822, 0.4465), (0.5, 0.5, 0.5)
opt.data.image_std = (0.5, 0.5, 0.5)       # image standard difference value  (0.2023, 0.1994, 0.2010)
#opt.data.image_std = (0.2023, 0.1994, 0.2010)       # image standard difference value  (0.2023, 0.1994, 0.2010)
...
opt.train.mode = 'self-sup+ms+fd+pers+vgg19_v11'                 # train generator mode [self-sup/none], [self-sup/cross-deconv/self-sup-ms]
...
opt.train.total_ep = 50  # 250, MIT-50
...
opt.model.gen.output_dim_s = 3              # number of dimension in residual layer
...
opt.model.gen.fd_cosw = 0.99                # for DivergenceLoss computation
opt.model.gen.fd_normw = 0.01
opt.model.gen.p_cosw = 0.8                  # for perspectiveLoss computation
opt.model.gen.p_normw = 0.2
...
opt.optim.n_iter_decay = 1  # 30                # iterations to linearly decay learning rate to zero [100]
...
opt.optim.lr_g = 0.00004 #0.0001                     # initial learning rate for generator, [0.0001]
...
opt.optim.gradient_w = 1.5                  # weight of image gradient loss, [1.0]

opt.optim.preserve_info_w = 1.0             # try to constrain the tune
...
opt.model.gen.div_detail_dict_equal = {'low':1.0, 'mid':1.0, 'deep': 1.0, 'out':1.0}

opt.optim.divergence_w = 1.5                # 'fd', weight of feature divergence loss, [1.0]
opt.optim.perspective_w = 1.0               # 'pers', feature perspective loss
