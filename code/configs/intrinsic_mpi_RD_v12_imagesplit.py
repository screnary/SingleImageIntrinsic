...
opt.gpu_ids = [2]                           # gpu ids, [0/2/3] or their combination
...
opt.data.split = 'imageSplit'                       # [imageSplit, sceneSplit]
...
opt.train.mode = 'v12-imagesplit_self-sup+ms+fd+pers'                 # train generator mode [self-sup/none], [self-sup/cross-deconv/self-sup-ms]
...
opt.train.total_ep = 160  # 250, MIT-50
...
opt.model.gen.output_dim_s = 3 #1              # number of dimension in residual layer
...
opt.optim.identity_w = 30.0                  # weight of image identity loss
opt.optim.ssim_w = 0.5 # 0.5                      # weight of image SSIM loss
opt.optim.gradient_w = 1.5  #1.5                  # weight of image gradient loss, [1.0]

opt.optim.preserve_info_w = 0.0 #1.0             # try to constrain the tune

opt.optim.divergence_detail = [0., 0., 0., 1., 2.] #[0.2, 0.4, 1.0, 1.0, 2.0]  # [low, mid, deep, out]
opt.optim.div_detail_dict = {'low':0., 'mid':0., 'mid2':0., 'deep': 0.2, 'out':
                             0.8}
opt.optim.div_detail_dict_equal = {'low':1.0, 'mid':1.0, 'mid2':1.0, 'deep': 1.0, 'out':1.0}
...
opt.optim.divergence_w = 0.1 #1.5                # 'fd', weight of feature divergence loss, [1.0]
...
opt.optim.lambda_i_w = 2.5                  # weight for cycle loss (I -> B, R -> I), [default 10.0] [1.0]
