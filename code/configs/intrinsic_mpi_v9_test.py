...
opt.data.use_norm = True                            # whether use normalization [True/False]
opt.data.batch_size = 2                             # batch size [8]
opt.data.batch_size_test = 1
opt.data.load_size = 436                            # loading image size
opt.data.new_size = 336                             # the output of dataset's image size
...
opt.train.mode = 'self-sup+ms+fd+pers+vgg19_v9_test'                 # train generator mode [self-sup/none], [self-sup/cross-deconv/self-sup-ms]
...
opt.optim.lr_g = 0.00001                     # initial learning rate for generator, [0.0001]
...
opt.optim.gamma = 0.05                       # how much to decay learning rate
...
opt.optim.div_detail_dict = {'low':0.05, 'mid':0.10, 'deep': 0.25, 'out':0.6}
...
opt.optim.lambda_b_w = 2.5                  # weight for cycle loss (B, R -> I -> B, R) B
