...
opt.train.mode = 'v8-3_self-sup+ms'
# opt.train.mode = 'v8_self-sup+ms+fd+pers'                 # train generator mode [self-sup/none], [self-sup/cross-deconv/self-sup-ms]
...
opt.train.total_ep = 200  # 250, MIT-50
...
opt.model.gen.n_layers = 4                  # number of conv blocks in decoder [4]
...
opt.model.gen.fd_cosw = 0.5  # 0.3 #0.99                # for DivergenceLoss computation
opt.model.gen.fd_normw = 0.1 # 0.1 #0.01
...
if 'MPI' in opt.data.name:
    opt.output_root = '../features-'+opt.train.trainer_mode+'-'+opt.train.mode+'-'+opt.data.name+'-'+opt.data.split \
...
opt.optim.n_iter_decay = 30  # 30                # iterations to linearly decay learning rate to zero [100]
...
opt.optim.divergence_detail = [0.2, 0.4, 1.0, 1.0, 2.0]  # [low, mid, deep, out]
opt.optim.div_detail_dict = {'low':0.00, 'mid':0.01, 'mid2':0.1, 'deep': 0.7,
                             'out': 1.0}
...
opt.optim.divergence_w = 0.1 if 'fd' in opt.train.mode else 0.0 #1.5                # 'fd', weight of feature divergence loss, [1.0]
opt.optim.perspective_w = 1.0 if 'pers' in opt.train.mode else 0.0 #1.0               # 'pers', feature perspective loss
