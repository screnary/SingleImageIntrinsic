...
def rgb_to_chromaticity(rgb):
    """ BUG: should divided by mean rather than sum !!!"""
    """ converts rgb to chromaticity """
    irg = np.zeros_like(rgb)
    # s = np.sum(rgb, axis=-1) + 1e-6
    s = np.mean(rgb, axis=-1) + 1e-6  # modified by [wzj-201906]
...
class IIW_ImageFolder(data.Dataset):
    ...
    def iiw_loader(self, img_path):
        ...
        img_path = self.root + "/datasets/IIW/data/" + img_path
        img = np.float32(io.imread(img_path)) / 255.0
    ...
    def __getitem__(self, index):
        ...
        img_path = self.root + "/datasets/IIW/" + self.img_list[self.current_o_idx][index]
        judgement_path = self.root + "/datasets/IIW/data/" + img_path.split('/')[-1][0:-6] + 'json'

        mat_path = self.root + "/datasets/IIW/long_range_data_4/" + img_path.split('/')[-1][0:-6] + "h5"
        ...
        if self.current_o_idx == 0:
            sparse_path_1s = self.root + "/datasets/IIW/sparse_hdf5_S/" + sparse_shading_name + "/R0.h5"
